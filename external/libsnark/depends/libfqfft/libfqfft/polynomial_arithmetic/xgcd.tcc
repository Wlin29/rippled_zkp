...
void _polynomial_xgcd(const std::vector<FieldT> &a, const std::vector<FieldT> &b, std::vector<FieldT> &g, std::vector<FieldT> &u, std::vector<FieldT> &v)
{
    ...
    std::transform(G.begin(), G.end(), G.begin(), [&](const FieldT& x){ return lead_coeff * x; });
    std::transform(U.begin(), U.end(), U.begin(), [&](const FieldT& x){ return lead_coeff * x; });
    std::transform(V1.begin(), V1.end(), V1.begin(), [&](const FieldT& x){ return lead_coeff * x; });
